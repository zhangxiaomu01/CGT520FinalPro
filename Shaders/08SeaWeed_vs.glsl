...
void main(void)
{
   ...
   float offset = 0.1*sin( 0.1 *time*pos_attrib.x);
	...
	if(pos_attrib.y < 0.8f)
	{
		offset = 0.14 * sin(0.2*time + 0.3* pos_attrib.z) + 0.015*cos(0.3* time*pos_attrib.y);
	}
	else
	{
		offset = pos_attrib.y*0.08 * sin(0.3*time + 0.4* pos_attrib.z) + 0.015*cos(0.3* time*pos_attrib.y) +
			0.2 * sin(0.1*time + 0.2* pos_attrib.z) ;
	}
	
   gl_Position = PVM*vec4(pos_attrib.x + offset, pos_attrib.y +offset01, pos_attrib.z + offset, 1.0);
