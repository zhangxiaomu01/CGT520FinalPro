...
void BufferIndexedVerts(MeshData& meshdata)
{
	...
	if (meshdata.mTangentSpace != -1)
	{
		glDeleteBuffers(1, &meshdata.mTangentSpace);
	}
	if (meshdata.mBitangetSpace != -1)
	{
		glDeleteBuffers(1, &meshdata.mBitangetSpace);
	}
	...
	tangent_loc = glGetAttribLocation(program, "tangent_attrib");
	mBitangent_loc = glGetAttribLocation(program, "mBitangent_attrib");
	...
	//Load tangent attributes
	if (mesh->HasTangentsAndBitangents())
	{
		//std::count << "we have tangent" << std::endl;
		//printf("we have tangent");
		float *mTangent = (float *)malloc(sizeof(float) * 3 * mesh->mNumVertices);
		for (unsigned int k = 0; k < mesh->mNumVertices; ++k) {
			mTangent[k * 3] = mesh->mTangents[k].x;
			mTangent[k * 3 + 1] = mesh->mTangents[k].y;
			mTangent[k * 3 + 2] = mesh->mTangents[k].z;
		}
		glGenBuffers(1, &meshdata.mTangentSpace);
		glBindBuffer(GL_ARRAY_BUFFER, meshdata.mTangentSpace);
		glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * mesh->mNumVertices, mTangent, GL_STATIC_DRAW);
		glEnableVertexAttribArray(tangent_loc);
		glVertexAttribPointer(tangent_loc, 3, GL_FLOAT, 0, 0, 0);
		free(mTangent);


		float *mBitangent = (float *)malloc(sizeof(float) * 3 * mesh->mNumVertices);
		for (unsigned int k = 0; k < mesh->mNumVertices; ++k) {
			mBitangent[k * 3] = mesh->mBitangents[k].x;
			mBitangent[k * 3 + 1] = mesh->mBitangents[k].y;
			mBitangent[k * 3 + 2] = mesh->mBitangents[k].z;
		}
		glGenBuffers(1, &meshdata.mBitangetSpace);
		glBindBuffer(GL_ARRAY_BUFFER, meshdata.mBitangetSpace);
		glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * mesh->mNumVertices, mBitangent, GL_STATIC_DRAW);
		glEnableVertexAttribArray(mBitangent_loc);
		glVertexAttribPointer(mBitangent_loc, 3, GL_FLOAT, 0, 0, 0);
		free(mBitangent);
...
MeshData LoadParticles(const std::string& pFile, glm::vec3 pos[])
{
	...
	BufferIndexedVertsParticles(mesh,pos);
	mesh.mNumIndices = mesh.mScene->mMeshes[0]->mNumFaces * 3;
	return mesh;
}
...
void BufferIndexedVertsParticles(MeshData& meshdata, glm::vec3 pos[])
{
	...
	//bind initial location
	GLuint instanceVBO;
	glGenBuffers(1, &instanceVBO);
	glBindBuffer(GL_ARRAY_BUFFER, instanceVBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 10, &pos[0], GL_STATIC_DRAW);
	glEnableVertexAttribArray(offset_loc);
	glVertexAttribPointer(offset_loc, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
